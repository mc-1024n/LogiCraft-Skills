...
> 스킬 동봉 `scripts/contrast_checker.py` 로 검사한 텍스트/배경 색 조합 결과. 토큰은 DS 의 *기존* 값만 사용.
> 본문 AA 4.5:1 / 큰글씨 3:1 이 하한. 실패분은 더 진한 DS 토큰으로 교체했거나, 토큰 한계면 ⚠️ 로 보강 필요 표기.

| 조합 (텍스트 × 배경) | 토큰명 | 대비 | 판정 | 조치 |
|---|---|---|---|---|
| <예: 본문 × 카드배경> | <gray-900 × white> | <17.7:1> | AAA | OK |
| <예: muted × 배경> | <gray-500 × white> | <4.6:1> | AA | OK |
| ... | ... | ... | <FAIL 이면> | <gray-700 로 교체 / ⚠️ DS 토큰 보강 필요> |
