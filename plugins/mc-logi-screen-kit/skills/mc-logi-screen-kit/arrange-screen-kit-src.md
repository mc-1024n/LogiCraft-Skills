# arrange-screen-kit.mjs — 소스 캐리어 (배포-안전 단일 진실원)

> 이 파일은 `bin/arrange-screen-kit.mjs` 의 **정본 소스**다. 스킬 배포 시 `bin/` 이 누락돼도
> 이 `.md` 는 포함되므로, 스크립트가 없을 때 Phase 3 가용성 게이트가 이 코드블록을
> 그대로 `bin/arrange-screen-kit.mjs` 로 써서 재생성한다. **bin/ 을 수정하면 이 파일도 동기화할 것.**
