...
컨트롤러(실제 코드)가 키트 `api_endpoint` ITEM(설계 계약)과 **path·method·request·response·envelope·상태코드** 차원에서 정합하는지 검토. 키트가 진실원 — 코드가 키트 계약을 위반하면 `code_drift`, 키트 ITEM 에 대응 핸들러가 없으면 `coverage_gap`, 키트에 없는 핸들러는 `extra_code`.
