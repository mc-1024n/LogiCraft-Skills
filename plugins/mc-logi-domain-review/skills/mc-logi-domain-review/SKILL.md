...
## 노하우 — 로컬 LLM(opencode 등) 워커로 돌릴 때 (2026-08-06 실전, 10도메인×10차원)

### ★ 컨텍스트 한계가 설계를 강제한다
자체 호스팅 모델(glm-5.2)은 실효 컨텍스트가 **약 250~300K** 였고, 이를 넘으면 세션이 **조용히 물려**
아무 산출물도 못 낸다(에러도 안 뜨고 컨텍스트 수치가 그대로 멈춘다). 이번 라운드에서 **4회** 발생.

| 시도 | 워커당 범위 | 결과 |
|---|---|---|
| 도메인 1개 = 워커 1개 (10차원) | 도메인 전체 | ❌ 294~586K 물림, 산출물 0 |
| 3청크 (차원 3~4개씩) | 도메인의 1/3 | ❌ 254~306K 물림 (첫 차원만 끝냄) |
| **차원 1개 = 워커 1개** | 차원 1개 | ✅ 105~200K 안정, 48조합 완주 |

→ **스킬 원래 설계(도메인×차원 단위 auditor)를 지켜라.** 도메인 단위로 뭉치면 로컬 모델에선 반드시 터진다.

### 컨텍스트를 태우는 진짜 원인
- `SKILL.md` + `checklist.md` ≈ 27KB, `dimensions/*.md` 10개 합 ≈ 62KB → **읽기만 ~30K 토큰**
  → 담당 차원 룰 **하나만** 읽게 하라. SKILL.md 는 워커가 읽을 필요 없다(오케스트레이터가 요약해 전달).
- 나머지 200K+ 는 `list_items` 페이지네이션과 `get_item` 정독. `list_items` 는 항목마다 `change_summary`
  까지 실어주므로 수백 건 페이징하면 그것만으로 찬다 → **차원별 타입 allowlist** 로 조회 범위를 좁혀라.

### ★ 판정마다 즉시 파일 append
"다 읽고 마지막에 쓴다"는 이 환경에서 구조적으로 실패한다. 물리면 전부 잃는다.
판정 1건마다 리포트에 append 시키면, 물려도 그때까지가 남는다(실제로 D004 부분결과를 그렇게 건졌다).
...
3. **"토큰 효율" 회피** — 전수 fetch 를 안 하고 `unable_to_verify` 로 처리하거나 "열거 생략"으로 뭉갠다.
   토큰 제약이 없으면 그렇게 못하도록 명시하고, `unable_to_verify` 는 *도구로도 확인 불가*할 때만 쓰게 하라.

### 심각도 정규화 없이 도메인 비교하면 왜곡된다
`stale` 계상 기준이 워커마다 갈렸다 — 같은 성격을 D005 는 0건, D004 는 **P0 26건**으로 셌다.
`stale=true` 는 증상이지 결함이 아니다. **본문을 열어 구모델 서술이 남은 것만 P0/P1**, 플래그만 보고 올린 건 P2.
실측 재판정 결과 D004 의 P0 26건은 **전부 0건**(cascade 완료·플래그만 잔존)이었다.
→ 정규화 기준을 파일로 만들어 통합 워커에 강제하라. 안 하면 P0 47건이라는 거짓 위기감이 남는다.

### 통합은 LLM 에 맡기지 말고 기계 추출하라
통합 워커에 "전건 수록"을 지시해도 **328행 중 31행만** 옮겼다(초기 시도).
gap 은 이미 각 리포트에 마크다운 표로 있으니 **스크립트로 전 행을 추출**해 원장(TSV)을 만들고,
LLM 분할본은 *사람이 읽는 용도*로 병행하라. 둘이 비슷한 수에 수렴하면 교차검증이 된다
(실측: 기계 204행 vs LLM 정독 205행 — D007/008/010/011 범위).

### 감사 자체를 감사하라 (커버리지 비평)
독립 read-only 워커에게 리포트 전체를 주고 ① 100조합 중 미수행 ② 스코프 누수 ③ 부당한
`unable_to_verify` ④ 근거 부실 gap ⑤ 도메인 간 모순을 잡게 하라. 이번에 실제로
"D003 16건이 토큰 효율 사유", "D004 P0 26건이 추정 언어" 를 잡아내 보완 감사로 이어졌다.
