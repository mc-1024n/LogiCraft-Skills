...
COV-7/8/9 는 도메인 description 을 **단일 진실원**으로 사용한다. 검사 전 도메인 책임 인벤토리를 추출:
...
#### COV-10: 비-HTTP 경계 계약(SVC/IAPI/LIB) 매핑 — COV-5 의 확장
- 도메인이 `service_interface`(MCP 도구·CLI)·`module_api`(in-process API)·`library_api` 를 보유하면
  API 와 동급 경계 계약으로 취급해 매핑을 검사한다 (경계 4종: API/SVC/IAPI/LIB)
- SVC/IAPI: 어떤 활성 DFEAT 의 `implements` 링크에도 안 붙은 활성 SVC/IAPI → **P2** gap
  (⚠️ P0 아님 — SVC 는 타 도메인 DFEAT 가 진실원인 운영 사례가 있으므로(예: D020 SVC ↔ D003 DFEAT)
  cross-domain implements 를 먼저 확인하고, 그래도 없으면 gap. evidence 에 cross-domain 확인 결과 명시)
- LIB: `implemented_by` MOD 미연결 활성 LIB → **P2** (구현 추적 부재)
- 검출: 도메인 SVC/IAPI 목록 vs 전 프로젝트 DFEAT.implements 합집합 차집합
...
- `D<NNN>-COV-010`: 비-HTTP 경계 계약(SVC/IAPI/LIB) 매핑 없음
...
- COV-010 경계 계약 매핑 없음 → `false` (DFEAT implements 연결 또는 cross-domain 진실원 확인 = 사용자)
