...
통합(integration)·시스템(system) 시험 시나리오(`test_scenario`, TEST-NNN)가 도메인의 핵심 흐름(cross-UC end-to-end)·책임 요구사항(REQ)·NFR 을 **빠짐없이·올바르게** 검증하는지 + `steps[]` 본문의 **현행성**(폐기·옇 흐름 검증 여부)을 검토.
...
2. **현행성(stale)**: TEST steps 가 폐기 ITEM·옇 흐름/화면/API 를 검증하고 있지 않은가
...
#### TST-005: steps 가 현행 모델과 불일치 (옇 흐름/화면/API) → **P1**
...
**gap 예시**: "TEST-008.steps[1] expected '200 {batch_id, replay} — Idempotency-Key 검증' ↔ UC-039 현행 main_flow 'body.batch_id 멱등(Idempotency-Key 폐기)' — 옇 멱등 모델 검증"
...
- kind=integration 인데 `covers_use_cases` 빈 / kind=system 인데 `verifies_requirements`·`verifies_nfrs` **둘 다** 빈 / `steps[].expected` 부재 / `objective` 빈약.
