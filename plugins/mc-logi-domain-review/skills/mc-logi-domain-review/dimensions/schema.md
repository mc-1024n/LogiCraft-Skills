...
### SCH-10: 비-HTTP 경계 계약(SVC/IAPI/LIB) 필드 정합 — 도메인이 해당 타입 보유 시
- `service_interface`: request_schema/response_schema/error_model 존재 여부 (계약인데 스키마 비면 P1)
- `module_api`: **required 인 owner_module·language** 채움 + methods 시그니처 존재 (P1)
- `library_api`: exports 비어있으면 P1 (공개 심볼 없는 라이브러리 계약은 무의미)
- 정확한 필수 필드는 `get_item_schema(type)` 진실원으로 확인 — 위는 대표 검사점
