...
큰 파일(5MB)는 토큰 폭발로 등록 불가다. 이 스킬은 결정적 스크립(`bin/upload-artifact.mjs`)이
...
- 파일 있는지 확인(Glob/Read 로 존재만, 본문 통독 불필요 — 본문은 스크립이 읽는다).
- project_id 는 `list_projects`(MCP) 또는 사용자 확인. server base 는 그 프로젝트가 있는 서버.

### Phase 2 — 업로드 실행
스크립 위치는 설치 무관하게 `Glob("**/mc-logi-upload-kit/bin/upload-artifact.mjs")` 로 탐색 후:
```bash
...
## 보안 메모
- 업로드된 본문도 **서버가 기존 sanitize + 봉인(ADR-025) 파이프라인을 그대로 경유** — 스크립은 전송만.
