...
## service_interface (SVC) — 비-HTTP 서비스 계약 (MCP 도구·CLI·데몬)

```markdown
## 서비스 계약 (구현: 도구/서비스 핸들러)
- protocol / delivery / direction / encoding: <원문 그대로>
- signature 또는 endpoint: <호출 시그니처>
- request_schema: <필드: 타입, 필수, 검증> (JSON 예시)
- response_schema: <스키마 + JSON 예시>
- message_schema: <메시지 기반이면>
- error_model: <에러 코드/조건 표 — E_* prefix 등 원문 그대로>
- interaction / triggers: <동기·비동기, 발행 이벤트>
- implements_features / verifies_acceptance: [FEAT-... / AC-...]
- **적용 상수**: uses_constants=[CONST-...] — 값은 CONST 그대로
```

## module_api (IAPI) — in-process 앱 모듈 공개 API

```markdown
## 모듈 공개 API (구현: 모듈 인터페이스 — local-first 앱의 경계)
- kind / language / runtime: <원문>
- owner_module: [MOD-...] — 이 API 를 소유한 코드 모듈 (required)
- methods: <메서드별 시그니처/파라미터/반환/에러> — 코드 직역 가능 수준
- error_model / interaction / triggers: <원문>
- operates_on: [ERD 테이블] — persists_in 재사용 (데이터 접점)
- implements_features / verifies_acceptance: [FEAT-... / AC-...]
- **적용 상수**: uses_constants=[CONST-...]
```

## library_api (LIB) — 라이브러리 공개 API

```markdown
## 라이브러리 계약 (구현: 공개 심볼)
- language / target / abi: <원문>
- exports: <함수/클래스/타입별 시그니처>
- include_path / link_name / bindings: <소비자 관점 사용법>
- error_model: <에러 규약>
- implemented_by: [MOD-...] / implements_features / verifies_acceptance
- **적용 상수**: uses_constants=[CONST-...]
```

## data_pipeline (DP) — 배치/파이프라인

```markdown
## 파이프라인 (구현: 배치 잡/스트림)
- mode / trigger / orchestrator / delivery: <원문 — 스케줄·트리거 조건 그대로>
- sources → stages → sinks: <단계별 입력/변환/출력 — 실행 순서로>
- reads_from / writes_to: [ERD 테이블·EXTSYS]
- emits_events: [EVT-...] / deployed_on: [INFRA-...]
- data_management: <보존·재처리·멱등성 정책>
- realizes_features: [FEAT-...]
```

## permission_manifest (PMAN) — OS 권한 매니페스트 (desktop 프로파일)

```markdown
## 권한 매니페스트 (구현: entitlements/Info.plist/manifest 파일 직역)
- entries: <권한 키별 — key / 사유 / 요구 기능> 표 (매니페스트 파일에 그대로 반영)
- sandbox / hardened_runtime: <원문 — 빌드 설정 직결>
- requested_by_features: [FEAT-...] — 이 권한을 요구하는 기능
- notes: <심사·배포 주의>
```

## settings_schema (SETT) — 환경설정 스키마 (desktop 프로파일)

```markdown
## 설정 스키마 (구현: 설정 저장소 + 설정 UI 폼)
- backend: <저장 위치 — UserDefaults/plist/파일 등 원문>
- groups: <그룹별 설정 키 — key / 타입 / default / 검증> 표
- **적용 상수**: uses_constants=[CONST-...] — default 값은 CONST 그대로
- notes: <마이그레이션·동기화 주의>
```

> **code_module (MOD)**: 다운로드 대상에서 제외됨(core-item-set.md 제외 규칙) — 구현 결과의
> 사후 추적 문서라 구현 입력 스펙이 아니다. 별도 요약 템플릿 없음.
