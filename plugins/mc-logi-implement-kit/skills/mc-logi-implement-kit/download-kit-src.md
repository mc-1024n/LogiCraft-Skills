...
> 이 `.md` 는 포함되므로, 다운로더가 없을 때 Phase 3 가용성 게이트가 이 코드블록을
 ...
 *        [--types adr,domain_feature] [--dry-run]
 *
...
async function main() {
  ...
  // 1) 경량 manifest (본문 없이 델타 판별)
  const light = await exportCall({
    domain,
    types,
    include_bodies: "false",
  });
  const serverItems = Array.isArray(light.items) ? light.items : [];
  ...
  // 삭제 감지 — 이번 필터 범위(같은 domain/types)에서 로컬에 있으나 서버에 없는 것
  ...
  const deleted = Object.entries(local.items)
    .filter(([id, l]) => inScope(l) && !serverIds.has(id))
    .map(([id]) => id);
  ...
  let bodies = [];
  if (changed.length) {
    const full = await exportCall({
      domain,
      types,
      ids: changed.map((c) => c.id).join(","),
      include_bodies: "true",
    });
    bodies = Array.isArray(full.items) ? full.items : [];
  }
  ...
  console.log(
    `✅ SYNC 완료 — 기록 ${wrote}건${deleted.length ? ` · 삭제 ${deleted.length}` : ""} · ${secs}s · 무열화 검증 통과`,
  );
}
